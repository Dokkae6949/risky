...
use crate::arch::consts::{get_heap_size, get_heap_start};

const PAGE_SIZE: usize = 1 << 12; // 4 KiB
...
pub fn init() {
    unsafe {
        assert_eq!(IS_INITIALIZED.load(Ordering::SeqCst), false, "the page allocator has already been initialized");

        let max_pages = get_heap_size() / PAGE_SIZE;
        ...
        ALLOC_START = align_up(get_heap_start() + max_pages * size_of::<Page>(), PAGE_SIZE);
...
/// # Safety
/// This function must only be called after the allocator has been initialized.
...
pub fn alloc(pages: usize) -> Option<*mut u8> {
    unsafe {
        assert_eq!(IS_INITIALIZED.load(Ordering::SeqCst), true, "the page allocator was not initialized");
    }
    assert!(pages > 0);
...
pub fn dealloc(ptr: *mut u8) {
    ...
    unsafe {
        let addr = get_heap_start() + (ptr as usize - ALLOC_START) / PAGE_SIZE;
...
pub fn print_page_allocations() {
    unsafe {
        let num_pages = get_heap_size() / PAGE_SIZE;
